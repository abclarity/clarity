...
(function(window) {
  ...
  // === Monat-Totals aus Storage laden ===
  function monthTotalsFromStorage(y, mIdx, INPUT_KEYS, activeFunnelId) {
  ...
  function fillYearAndQuarter(y, YEAR_COLUMNS, INPUT_KEYS, KPI_COLS, activeFunnelId) {
    ...
    for (let m = 0; m < 12; m++) {
      const { totals } = monthTotalsFromStorage(y, m, INPUT_KEYS, activeFunnelId);
    ...
    Object.entries(quarters).forEach(([label, months]) => {
      const tr = document.createElement("tr");
      ...
      months.forEach(m => {
        const { totals, hasAny: monthHas } = monthTotalsFromStorage(y, m, INPUT_KEYS, activeFunnelId);
        if (monthHas) {
